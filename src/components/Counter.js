...
const Counter = (props) => {
  ...
  return (
    <div className="counter">
      ...
      <div className="counter__memo__wrapper">
        <h3>Memo example using age</h3>
        <p>Current Age: <span>{age} </span></p>
        <p>Current SquaredAge: <span>{currentAge}</span></p>
        <p>Memoized SquaredAge: <span> {memoizedAge}</span> </p>
        <button
          onClick={() => {
      ...
      <div className="counter__word__wrapper">
        <h3>Compute number of letters (slow )</h3>
        <p>
          The word:<span>"{word}"</span>  has <span>{letterCount}</span> letters
        </p>
        <button
          onClick={() => {
